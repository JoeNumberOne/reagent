...
def get_graph_feature(x, k=20, idx=None, dim6=False):
    ...
    feature = feature.view(batch_size, num_points, k, num_dims)  #[4,717,20,3]
    x = x.view(batch_size, num_points, 1, num_dims).repeat(1, 1, k, 1) #x[4,717,20,3]
...
class AdaptiveConv(nn.Module):
    ...
    def forward(self, points, feat, idx):
        # points: (bs, in_channels, num_points)[4,3,717], feat: (bs, feat_channels/2, num_points) [4,64,717]
        batch_size, _, num_points = points.size() #points[4,3,717]
        #batch_size=4,num_points=717
        x, _ = get_graph_feature(points, k=self.k, idx=idx)  # (bs, in_channels, num_points, k)[4,6,717,20]
        y, _ = get_graph_feature(feat, k=self.k, idx=idx)   # (bs, feat_channels, num_points, k)[4,6,717,20]

        kernel = self.conv0(y)  # (bs, nhiddens, num_points, k) [4,64,717,20]
        ...
        kernel = kernel.permute(0, 2, 3, 1).view(batch_size, num_points, self.k, self.nhiddens,
                                                 self.in_channels)  # (bs, num_points, k, nhiddens, in)[4,717,20,64,6]

        x = x.permute(0, 2, 3, 1).unsqueeze(4)  # (bs, num_points, k, in_channels, 1)
        x = torch.matmul(kernel, x).squeeze(4)  # (bs, num_points, k, nhiddens)
        x = x.permute(0, 3, 1, 2).contiguous()  # (bs, nhiddens, num_points, k)[4,64,717,20]

        # nhiddens -> out_channels
        x = self.leaky_relu(self.bn1(x))
        x = self.linear(x)  # (bs, out_channels, num_points, k)[4,64,717,20]
        # residual: feat_channels -> out_channels
        residual = self.residual_layer(y) #4,64,717,20]
        x += residual
        x = self.leaky_relu(x)

        x = x.max(dim=-1, keepdim=False)[0]  # (bs, out_channels, num_points) [4,64,717]
...
class AdaptiveFCN(nn.Module):
    def __init__(self, neighboursnum, emb_dims=512):
        ...
        self.neighboursnum = neighboursnum  #20
    ...
    def forward(self, x):
        x = x[:, :, 0:3]              #[32 1024 3]
        x = x.permute(0, 2, 1).contiguous()  #[32 3 1024]# (B, 3, N)[4,3,717]
        ...
        x1 = self.adp_conv1(points, x, idx)  #[32 64 1024] 返回[bs out_channels num_points]
        x2 = self.adp_conv2(points, x1, idx) #[32 64 1024]
        x3 = self.adp_conv3(points, x2, idx) #[32 128 1024]
        x4 = self.adp_conv4(points, x3, idx) #[32 256 1024]

        x = torch.cat((x1, x2, x3, x4), dim=1) #[32 512 1024]
        x_node = x #[32 512 1024]

        x_edge = F.relu(self.bn(self.conv(x.unsqueeze(dim=-1)))).view(batch_size, -1, num_points) #[32 512 1024]
        #[4,512,717]
        x = torch.cat((x_node, x_edge), dim=1).transpose(1, 2).contiguous() #[32 1024 1024]
        return x